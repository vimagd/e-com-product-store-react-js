...
const ProductTitle = (props) => {
    ...
    return (<>
        <main className="title-section">
            <p className="title-title">{proTit.title.slice(0, 20)}...</p>
            <p className="title-price">${proTit.price}</p>
            <figure className="title-rating-rate">{ratingMethod()}<span className="title-rating-count">({proTit.rating.count})</span></figure>
            <p className="title-description">{readToggle ? proTit.description : proTit.description.slice(0, 80)} <span className="title-rating-count" style={{ cursor: "pointer" }} onClick={() => setReadToggle(!readToggle)}>{readToggle ? "Less" : "Read more"}</span></p>
